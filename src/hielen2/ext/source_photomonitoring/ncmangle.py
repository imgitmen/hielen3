...
class NCMangler():
    ...
    def __init__(self, targetfile=None,dims=None,reftime=None,gridratio=None,offset=None,**kwargs):
        """
        ...
        if not os.path.exists(targetfile):
            ...
            y_values=np.arange(0,dims[1])*gridratio+offset[1]
            x_values=np.arange(0,dims[0])*gridratio+offset[0]

            rootgrp = Dataset(targetfile, 'w', format="NETCDF4")
            rootgrp.Conventions="CF-1.7"

            rootgrp.reftime= str(np.datetime64(reftime))

            # time informations
            rootgrp.createDimension("time", None)
            rootgrp.createVariable("time","f8",("time",))
            rootgrp.variables["time"].units=f"hours since {reftime}"
