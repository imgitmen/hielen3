...
def get_ch(
    ...
    return GWO(path, restype, filename).getDataSeries(
        resource=resource, column=column, timefrom=timefrom, timeto=timeto
    )
...
class GWO:
    ...
    def getDataSeries(self, resource=None, column=None, timefrom=None, timeto=None):
        ...
        out = read_csv(
            f"{self.path}/{self.restype}/{resource}/{self.filename}",
            header=None,
            index_col=[0],
        )[column]
